-- Applied to production 2026-08-14 via the dashboard SQL editor (no Supabase MCP in session);
-- NOT recorded in supabase_migrations.schema_migrations. Idempotent — re-applying is harmless.
-- import_scraped_listings: stop writing properties.usable_acres.
-- The 2026-08-13 uplands migrations made usable_acres a STORED GENERATED column
-- (derived from wet_acres_nwi / lowlands_acres_county), so any INSERT/UPDATE that
-- names it fails with 428C9 — which killed every scrape ingest path since 08-13.
-- The r->>'usable_acres' input is intentionally ignored from now on.
