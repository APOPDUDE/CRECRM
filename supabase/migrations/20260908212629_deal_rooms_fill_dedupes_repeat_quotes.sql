-- An asking comp is re-minted every time the sweep sees the listing, so one space
-- can carry a dozen rows at the same rate. 9610's asking set was 25 rows but only
-- ~11 distinct listings -- 7908 Professional Plz alone appeared 10 times. Keep one
-- row per (property, deal type, size), the most recent quote.
...
begin
  ...
  with pick as (
    select cp.id,
           pr.id as prop_id,
           cp.deal_type,
           coalesce(cp.sf, pr.gross_sf) as size_sf,
  ...
  deduped as (
    select distinct on (prop_id, deal_type, size_sf) *
    from banded
    order by prop_id, deal_type, size_sf, dated desc nulls last
  )
