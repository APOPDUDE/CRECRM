-- Stop throwing away the scraper's building facts.
--
-- The LoopNet actor returns a rich `propertyFacts` object (ClearHeight, TotalDockDoors,
-- DriveInBays, Levelers, CrossDocks, TruckCourt, SprinklerSystem, ColumnSpacing,
-- ConstructionMaterial, StandardParkingSpaces, PowerSupply...). Almost all of it was being
-- dropped twice over: n8n's map1 only lifted BuildingSize/LotSize/YearBuiltRenovated, and
-- import_scraped_listings has a fixed column whitelist that ignored anything else.
--
-- Typed columns below for the facts worth filtering and displaying on, PLUS a raw
-- `scrape_facts` jsonb so a fact we haven't modelled yet is never lost again -- future
-- needs become a read of existing data instead of a re-scrape.

alter table properties
  add column column_spacing text,           -- "50' x 50'"
  add column dock_levelers integer,         -- dock doors fitted with a leveler
  add column cross_docks boolean,
  add column parking_spaces integer,        -- a count; parking_ratio stays the per-1k ratio
  add column construction_material text,    -- "Reinforced Concrete"
  add column truck_court_ft numeric(6,1),   -- "120'"
  add column sprinkler_system text,         -- "ESFR"
  add column amps integer,                  -- from PowerSupply "Amps: 2,000"
  add column scrape_facts jsonb;            -- raw propertyFacts, verbatim

alter table properties
  add constraint properties_dock_levelers_nonneg check (dock_levelers is null or dock_levelers >= 0),
  add constraint properties_parking_spaces_nonneg check (parking_spaces is null or parking_spaces >= 0),
  add constraint properties_truck_court_nonneg check (truck_court_ft is null or truck_court_ft >= 0),
  add constraint properties_amps_nonneg check (amps is null or amps >= 0);

comment on column properties.scrape_facts is
  'Raw propertyFacts blob from the listing scrape, kept verbatim so unmodelled facts survive.';
comment on column properties.parking_spaces is
  'Count of parking spaces. Distinct from parking_ratio, which is spaces per 1,000 SF.';

-- Teach the importer about every new column. Everything else in the function is unchanged;
-- the whitelist is the only reason these never landed.
...
begin
  ...
  for r in select value from jsonb_array_elements(coalesce(p_props,'[]'::jsonb)) as t(value) loop
    ...
    insert into properties (
      ...
      usable_acres, dock_high_doors, grade_level_doors, clear_height_ft, three_phase_power, volts,
      ...
      nullif(r->>'usable_acres','')::numeric, nullif(r->>'dock_high_doors','')::int, nullif(r->>'grade_level_doors','')::int,
    ...
    do update set
      ...
      -- new building facts: a fresh scrape fills gaps but never blanks a value you typed
      usable_acres=coalesce(excluded.usable_acres, properties.usable_acres),
      ...
      -- raw snapshot: newest scrape wins outright
