-- Extends import_scraped_listings: every scraped LEASE listing (one with an asking
-- $/SF/yr) also upserts an asking lease comp (source='scrape') keyed on the namespaced
-- source_listing_id. Executed-deal fields stay null (manual only, enforced by the
-- lease_comps_executed_is_manual CHECK). Sale listings (no asking_rate_psf) make no comp.
...
begin
  ...
  loop
    ...
    -- asking lease comp from a scraped lease listing (has an asking $/SF/yr + a dedupe key)
