...
--
-- Two amendments to import_county_parcels for the land-book county download
-- (Alex 2026-08-21: "go through all the counties again and download the land"):
--
-- 1. LAND ROWS OFTEN HAVE NO SITUS ADDRESS. The old rule skipped any row
--    whose address lacks a digit ("can never be mapped or called on") — right
--    for buildings, wrong for vacant land: it CAN be mapped (the county layer
--    gives a centroid) and called on (owner mailing address). Rows with no
--    usable situs but WITH coordinates now insert under the established
--    'Parcel <id>' placeholder (the same convention enrich-appraiser treats
--    as fillable). No situs AND no coords is still a skip.
--
-- 2. PARCEL DEDUPE MUST SURVIVE FORMAT DRIFT. Stored parcel_number formats
--    are mixed WITHIN counties (Pinellas '16 30 05 ...' vs '16-30-05-...',
--    Sarasota '0000-00-8166' vs '0000008166', Hillsborough dashed PIN vs
--    bare) — exact-match dedupe would mint duplicates for parcels the book
--    already holds. A second tier matches on parcel_key's first segment
--    (lower(alnum(parcel_number)), generated column, btree-indexed — the
--    LIKE 'x|%' prefix anchors the whole segment), before the address
--    fallback. Comma-list assemblage rows still fall through to the address
--    tier, as before.
...
begin
  ...
  loop
    ...
    -- No usable situs: land is still mappable (centroid) and callable (owner
    -- mail), so take the 'Parcel <id>' placeholder — but only WITH coords;
    -- no situs and no coords is a row nothing in the app could ever surface.
    if v_addr is null or v_addr !~ '\d' then
      if nullif(r->>'lat', '') is null or nullif(r->>'lng', '') is null then
        n_skip := n_skip + 1;
        continue;
      end if;
      v_addr := 'Parcel ' || v_parcel;
    end if;

    select id into v_id from properties
    where parcel_number = v_parcel and county = v_county limit 1;

    -- format-drift tier: same digits/letters, different punctuation.
    -- parcel_key = lower(alnum(parcel_number)) || '|' || digits(folio);
    -- the prefix LIKE anchors the whole first segment on its btree index.
    if v_id is null then
      v_norm := lower(regexp_replace(v_parcel, '[^a-zA-Z0-9]', '', 'g'));
      if v_norm <> '' then
        select id into v_id from properties
        where county = v_county and parcel_key like v_norm || '|%'
        limit 1;
      end if;
    end if;
