-- import_scraped_listings: file a listing under the building we already know.
--
-- Before: a property was keyed on the LISTING id (loopnet:NNN / crexi:NNN). A new listing
-- id for a building already in the book could only be bridged via loopnet_property_id
-- (populated on 240 of 4,585 scraped rows) or an exact, case-folded address string -- and
-- that claim refused rows that were already listings (the 2026-08-18 guard). So every
-- re-listing, every second-source listing with "Hwy" vs "Highway", and every county roll
-- row with the terse situs form minted another property. 3145 US Highway 92 E: four rows.
--
-- After: the listing key (v_key) and the property key (v_pkey) are separate. A listing is
-- filed under (a) the LoopNet building id, (b) the property its market_listings row already
-- points at (so a merge_properties survivor keeps collecting its listings), or (c) the
-- parcel / normalized-address match in the same city. Listing rows are joined, not re-keyed;
-- county / manual / owner_import rows are re-keyed as before. Comps and market_listings stay
-- keyed on the listing id. Nothing else in the body changed.
...
if not exists (select 1 from properties where source_key = v_pkey) then
  ...
  if v_claim is not null then
    if v_alias_key is null
       or (v_alias_key not ilike 'loopnet:%' and v_alias_key not ilike 'crexi:%') then
      -- a county / manual / owner_import row: it becomes this listing's property
      update properties set source_key = v_pkey where id = v_claim;
    else
      -- already a listing row: file this listing under it, keep its key
      v_pkey := v_alias_key;
    end if;
  end if;
end if;
...
-- the two lookups above must be index hits, not 32k-row scans per listing (8s statement timeout)
create index if not exists properties_parcel_alnum_idx on properties (split_part(parcel_key, '|', 1));
create index if not exists properties_address_norm_idx on properties (normalize_address_text(address));
create index if not exists properties_site_address_norm_idx on properties (normalize_address_text(site_address))
