-- A building has ONE asking sale price, but can be marketing several lease suites.
-- Keying the dedupe on size let 3221 Cherry Palm through twice (38,624 vs 38,600 SF
-- captured off the same listing). So: sales dedupe per property, leases per suite.
...
begin
  ...
  with pick as (
    select cp.id,
           ...
           cp.deal_type,
           cp.kind,
           -- sales collapse to one per building; lease suites stay distinct
           case when cp.deal_type = 'sale' then null
                else coalesce(cp.sf, pr.gross_sf) end as dedupe_sf,
           coalesce(cp.executed_at, cp.as_of_date) as dated,
  ...
  deduped as (
    select distinct on (prop_id, deal_type, kind, dedupe_sf) *
    from banded
    order by prop_id, deal_type, kind, dedupe_sf, dated desc nulls last
  )
  insert into deal_room_comps (deal_room_id, comp_id, sort_order)
  select p_deal_room_id, s.id, row_number() over (order by s.miles)
  from (select * from deduped order by miles limit p_limit) s
