-- 2026-09-03: smooth, dissolved flood zones and wetlands (Alex: "refine the polygons into
-- smooth shapes like Mapwise"). Applied via MCP.
--
-- Two things made the ground layers look jagged: FEMA's NFHL splits one zone into many
-- panel pieces with visible seams, and straight-line simplification leaves corners. Inside
-- the (padded) viewport, touching pieces of one class + BFE are dissolved into a single
-- shape and every shape is run through Chaikin curve smoothing after a light simplify.
-- NWI polygons carry their own Cowardin code and acres, so their props differ and each is
-- smoothed on its own rather than merged (adjacent NWI polygons of one code are already one
-- polygon). The line and point kinds keep the old per-feature path.
...
begin
  ...
  env_pad := extensions.st_expand(env, greatest(p_east - p_west, p_north - p_south) * 0.2);
