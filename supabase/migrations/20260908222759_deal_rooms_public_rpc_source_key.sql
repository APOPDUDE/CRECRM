-- Expose the property's own source_key so the page can offer a listing link.
-- It is a public LoopNet/Crexi listing id, and the client builds the URL with
-- listingUrlFromSourceKey() -- the one builder, never a scraped url field, which
-- has been observed carrying a NEIGHBOURING listing's id.
...
begin
  ...
  from (
    select
      ...
      coalesce(cp.sf, pr.gross_sf)               as sf,
      ...
      cp.opex_psf,
      cp.sale_price,
      round(coalesce(cp.price_per_sf,
                     cp.sale_price / nullif(pr.gross_sf, 0))::numeric, 2) as price_psf,
      cp.cap_rate_pct,
      coalesce(cp.executed_at, cp.as_of_date)    as executed_at,
  ...
  select jsonb_build_object(
      'lease_median_psf', (
        select round(percentile_cont(0.5) within group (order by x.psf)::numeric, 2)
        from (
          select coalesce(cp.executed_lease_rate_psf, cp.asking_lease_rate_psf) as psf
          from deal_room_comps drc join comps cp on cp.id = drc.comp_id
          where drc.deal_room_id = r.id and cp.deal_type = 'lease'
        ) x where x.psf is not null),
      'sale_median_psf', (
        select round(percentile_cont(0.5) within group (order by x.psf)::numeric, 2)
        from (
          select coalesce(cp.price_per_sf, cp.sale_price / nullif(pr2.gross_sf, 0)) as psf
          from deal_room_comps drc
          join comps cp on cp.id = drc.comp_id
          join properties pr2 on pr2.id = cp.property_id
