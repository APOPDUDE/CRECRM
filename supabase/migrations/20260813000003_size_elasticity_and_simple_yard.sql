-- Two corrections Alex caught by looking at one property.
--
-- 1. "the psf for the building is way too high" - 4325 Hwy 92 came out at $218/SF
--    on comps that mostly trade $89-152. The size-elasticity MEASUREMENT was
--    contaminated: no price band, land sales included, slivers and mega-boxes
--    included. It returned -0.500 for industrial sale (pinned to the clamp floor)
--    against a clean -0.196, so every small building was marked up ~55% instead of
--    ~19%. One 7,566 SF comp was being restated 60% higher for a 2,400 SF subject
--    and still carrying real weight. Band the prices, drop land, drop the size
--    tails, tighten the clamp to [-0.35, 0], and tighten the per-comp size
--    adjustment from [0.6, 1.6] to [0.75, 1.4]. 4325 Hwy 92 lands at $176/SF.
--
-- 2. "how does 4325 Hwy 92 be $27k an acre, that is way too low" - because the
--    price was being spread over all 25.94 acres, 21.49 of which are swamp. Per-acre
--    is now quoted on USABLE acres: $133K. (Card-side change; the engine already
--    returned everything needed.)
--
-- 3. Alex's simplification, having heard the double-count argument and decided:
--    the yard is uplands less the building's GROSS footprint. No parking/apron
--    deduction. Refitting the $/acre rate on that basis was tried and is unusable
--    (p25/p75 span millions - the county-median psf is too crude an anchor for a
--    single building once the denominator grows), so the measured rate stays and
--    the size decay absorbs the wider basis. On a low-coverage IOS site the two
--    bases barely differ (4.39 vs 4.19 ac here).
...
with q as (
  ...
  where coalesce(c.sf, p.gross_sf) between 800 and 500000
...
comment on view v_comp_size_elasticity is
  'Per property_type x lease|sale: regression of ln(rate) on ln(size) - the "small space costs more per foot" curve. Prices MUST be banded and land excluded or the slope runs away: unbanded it measured -0.5 for industrial sale against a clean -0.196, marking every small building up 55% instead of 19%. Clamped [-0.35, 0].';
...
alter table properties
  ...
  generated always as (
    case
      ...
      -- GROSS, not heated: heated is living area and would understate the footprint,
      -- which would overstate the yard. No storeys divisor - Alex asked for simple.
      else round(greatest(0, coalesce(usable_acres, land_acres)
                             - coalesce(gross_sf, heated_sf, 0)::numeric / 43560), 2)
    end
  ) stored;

comment on column properties.net_usable_acres is
  'Uplands minus the gross building footprint, in acres - the usable yard. Generated. This IS what the valuation prices as land (Alex 2026-08-13): no parking/apron deduction.';
...
begin
  ...
  -- Alex's definition: the yard is the uplands less the building's GROSS footprint.
  -- No parking/apron deduction - he asked for simple, having heard the double-count
  -- argument. subj_sf is coalesce(gross_sf, heated_sf), so gross wins where we have it.
  ...
  -- Sale value scales over the whole excess (decayed). Rent does NOT: only so many
  -- acres can be let as a yard, and beyond that the land is a development play.
  ...
  adjusted as (select s.*,(s.w_dist*s.w_size*s.w_time*s.w_kind*s.w_verified*s.w_type)::numeric as weight,
           case when s.bucket='land' then
                ...
                -- Tighter than it was: at [0.6,1.6] a 7,566 SF comp was restated 60%
                -- higher for a 2,400 SF subject and still carried real weight.
