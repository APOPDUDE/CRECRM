-- properties.description — broker-facing narrative for a property ("talked to the listing
-- broker, here's the story"). Editable in place on the property page, the prospect-card
-- preview (the showcase), and the match slide-over. The scrape import FILLS it from the
-- LoopNet listing blurb only when it's blank — a manually written description is never
-- overwritten by a sweep (note the reversed coalesce below vs. the other columns).
alter table public.properties add column if not exists description text;

-- Re-emit import_scraped_listings with description support. Identical to 20260708000001
-- except: description added to the insert column list / values, and
-- `description = coalesce(properties.description, excluded.description)` in the update.
...
begin
  ...
  for r in select value from jsonb_array_elements(coalesce(p_props,'[]'::jsonb)) as t(value) loop
    ...
    -- claim block: unknown key + same-address row -> adopt that row's identity
    v_claim := null;
    ...
    do update set
      ...
      -- description: existing text wins — a broker-written description is never clobbered
