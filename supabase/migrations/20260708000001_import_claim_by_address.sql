-- import_scraped_listings: unify incoming rows with existing properties by address when the
-- incoming source_key is unknown.
--
-- The memo23 LoopNet actor's late-June 2026 update dropped summary/parcelNumber from its
-- output, so scraped rows now always key by loopnet:<id> while many pre-existing rows are
-- keyed by parcel number. Without this, every previously-parcel-keyed property duplicates on
-- the next sweep (observed: 257 address-duplicates in the 2026-07-08 replay, rolled back via
-- _rollback_replay_20260708). When an incoming key matches nothing, the oldest row with the
-- same (address, city) is CLAIMED: its source_key is re-pointed to the incoming key (the
-- parcel_number column itself is untouched), then the normal on-conflict upsert path updates
-- that row. Placeholder addresses ('Parcel ...', 'Address unavailable') are never matched, so
-- distinct parcel-only rows can't be merged by their labels.
--
-- Only the marked "claim" block is new; the rest of the function body is unchanged from
-- 20260622000004 (fetched from the live definition).
...
begin
  ...
  for r in select value from jsonb_array_elements(coalesce(p_props,'[]'::jsonb)) as t(value) loop
    ...
    -- claim block (new): unknown key + same-address row -> adopt that row's identity
    v_claim := null;
...
-- keep the hardened ACLs self-documented (create or replace preserves them anyway)
grant execute on function public.import_scraped_listings(jsonb, uuid, boolean) to authenticated, service_role;
revoke execute on function public.import_scraped_listings(jsonb, uuid, boolean) from anon, public;
