-- Yard rent has a ceiling; land value does not.
--
-- Filling `usable_acres` from NWI (migration 20260813000000) exposed this: once a
-- 3,083-acre parcel reported 1,792 acres of dry excess, the per-acre yard rent
-- produced **$2,472,224 a month**. The rent decay alone was never going to hold -
-- the error is a category one. Nobody lets seventeen hundred acres as an IOS
-- yard; past a point the land is a development play and the yard is whatever
-- fraction of it a tenant fences.
--
-- So the RENT is billed on at most `land_rent_max_acres`. SALE value is NOT
-- capped - a big parcel really is worth more than a small one, and the measured
-- size decay already handles the per-acre fall-off.
--
-- Also threads `usable_acres_source` into land_component so the panel can say
-- whether the uplands were measured or assumed.

insert into valuation_params (key, value, notes) values
  ('land_rent_max_acres', 20.0,
   'Yard acreage that can actually be let as IOS. Past this the land is a development play, not a yard - a 1,792-acre "yard" priced per acre produced $2.5M/month. Sale value is NOT capped; the size decay handles it.')
...
begin
  select p.id,p.address,p.city,p.state,p.zip,p.county,p.lat,p.lng,
         ...
         p.usable_acres_source, p.just_value,p.assessed_value,
