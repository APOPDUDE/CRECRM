-- County deed records land as kind='transfer' and carry sale_price but never
-- price_per_sf, so the first cut of fill_comps missed every real sale comp and
-- kept only the handful of brokered 'executed' rows -- which skewed small and
-- retail. Sales now derive $/SF from the property's gross_sf, drop portfolio
-- trades (a 31-property allocation is not a comp), and sit inside a sane band.
...
drop function if exists deal_room_fill_comps(uuid,numeric,integer,integer,date,text,integer);
