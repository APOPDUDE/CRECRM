-- R7: import_scraped_listings — the asking comp becomes the listing-event record.
-- Two changes, everything else byte-identical to the 2026-08-14 live body
-- (the post-usable_acres-outage version):
--   1. The asking-comp branch fires for EVERY listing row, priced or not — an
--      unpriced listing ("rate upon request") creates/updates an asking comp with
--      null rate/price. This closes the 2026-07-27 gap where unpriced listings
--      left no record at all. Change-detection is unchanged: a re-scrape with
--      identical terms UPDATEs the latest comp instead of appending.
--   2. The comp insert/update carries the listing-event fields from the payload
--      (listing_url, broker_*, days_on_market, listed_at, title->listing_title,
--      description->listing_description, sale_conditions/status/type, is_auction,
--      occupancy, source_last_updated).
-- The properties columns keep being written exactly as before (dual-write until
-- the UI repoint deploys and the columns drop in a later block). Same payload
-- keys, same return shape, same claim logic. n8n map nodes untouched.
