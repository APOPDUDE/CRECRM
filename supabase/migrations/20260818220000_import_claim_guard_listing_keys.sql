-- Claim-by-address must never hijack a property that already carries a listing
-- identity (crossed listing-URL bug, 2026-08-18 — see
-- context/comp-sheet-listing-url-audit-2026-08-18.md).
--
-- Before: when a scraped listing's key had no property yet, the importer claimed
-- the first property matching the exact address+city and stamped source_key = v_key
-- — even when that property's source_key was already loopnet:/crexi: from a
-- DIFFERENT listing at the same address (two suites in one park, address ranges,
-- broker double-postings). Result: one property row accumulating comps from two
-- listings, source_key saying A while the newest comp's listing_url says B — 35
-- crossed LoopNet comps found in the 2026-08-18 audit.
--
-- After: county-created properties (parcel:/addr:/bare/null keys) are still
-- claimable — that linking is the feature — but a property already keyed to a
-- listing is left alone, so a second listing at the same address mints its own row.
-- ONLY the claim SELECT changes; everything else is the live definition verbatim.
