-- 2026-09-03: Flood zones + wetlands on the map (Alex: "add a flood zone toggle too and
-- wetlands if you can… correctly show where the wetlands are"). Applied via MCP.
--
-- Nothing new is harvested: fema_flood (FEMA NFHL, 272,710 polygons) and nwi_wetlands
-- (USFWS National Wetlands Inventory, 264,765) have been in the cache since 08-24 and are
-- exactly what parcel_enrichment.fema_flood_zone / pct_sfha / wetlands_pct were measured
-- from, so the map and the property page cannot disagree. What changes: the two sources
-- join the layer catalogue, feature_props learns their fields, and map_layer_features
-- serves them — flood zones without the unshaded-X "minimal hazard" filler (it is most of
-- the land and says nothing), and both only inside ~150 km² viewports (zoom 12 in): NWI at
-- county scale reads as "half of inland Florida is swamp" (the 08-16 lowlands verdict).
...
insert into gis.map_layers (source_id, kind, county, jurisdiction, sort) values
  ('fema_flood',   'flood_zone', null, 'FEMA NFHL',                          85),
  ('nwi_wetlands', 'wetland',    null, 'USFWS National Wetlands Inventory',  86)
...
-- flood zones: SFHA + 0.2% only (unshaded X is filler); ground layers inside ~150 km2
