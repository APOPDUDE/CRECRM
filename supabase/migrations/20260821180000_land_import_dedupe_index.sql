-- Applied 2026-08-21 via MCP, mid-land-import.
--
-- The format-drift dedupe tier (20260821170000) matched with
-- `parcel_key LIKE v_norm || '|%'`, counting on the btree prefix
-- optimization — which only exists under C collation / text_pattern_ops.
-- On this database's default collation the planner seq-scans, so every
-- imported row paid a full-table scan and pages slowed as the book grew:
-- 32k rows cleared fine, ~95k rows blew the 8s statement timeout (Polk
-- stalled at offset 31,200).
--
-- Fix: equality on split_part(parcel_key,'|',1) — the parcel_number segment —
-- backed by an expression index, which works under any collation. Also skip
-- the address tier for 'Parcel <id>' placeholder rows: a placeholder can
-- never legitimately address-match, so probing was pure cost.

begin;

create index if not exists properties_county_parcel_norm_idx
  on properties (county, split_part(parcel_key, '|', 1));

comment on index properties_county_parcel_norm_idx is
  'Format-drift parcel dedupe: (county, alnum-normalized parcel_number). Used by '
  'import_county_parcels tier 2.';
...
begin
  ...
  loop
    ...
    -- format-drift tier: same digits/letters, different punctuation. Equality
    -- on the parcel_key segment rides properties_county_parcel_norm_idx.
    if v_id is null then
      v_norm := lower(regexp_replace(v_parcel, '[^a-zA-Z0-9]', '', 'g'));
      if v_norm <> '' then
        select id into v_id from properties
        where county = v_county and split_part(parcel_key, '|', 1) = v_norm
        limit 1;
      end if;
    end if;

    -- address tier: never for placeholder addresses (nothing to match).
    if v_id is null and not v_ph then
      select id into v_id from properties
