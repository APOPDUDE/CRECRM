-- Local PostGIS cache schema (the sidecar database, NOT hosted Supabase).
-- Applied idempotently by `python -m enrichment init-db` on every run.
--
-- Layers and parcel polygons are cached HERE; only derived scalars ever reach
-- Supabase (via import_parcel_enrichment) — see context/land-book-parcel-enrichment.md §4.
