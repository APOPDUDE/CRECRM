  ...
  return useQuery({
    ...
    queryFn: async (): Promise<DashMatch[]> => {
      const { data, error } = await supabase
        .from('matches')
        .select(
          'id, created_at, inquiry_date, tour_date, loi_date, lease_negotiation_date, execution_date, stage, flagged_new, actual_fee, tenant_rep_id, listing_id, property:properties!matches_property_id_fkey(id, address, city, state), tenant_company:companies!matches_tenant_company_id_fkey(name), tenant_contact:contacts!matches_tenant_contact_id_fkey(first_name, last_name), tenant_rep:tenant_reps!matches_tenant_rep_id_fkey(actual_fee), listing:listings!matches_listing_id_fkey(actual_fee)',
        )
...
/** Commission booked on an executed match — the match's own fee, else its parent's (legacy). */
export function matchFee(m: DashMatch): number {
  return m.actual_fee ?? m.tenant_rep?.actual_fee ?? m.listing?.actual_fee ?? 0
}
